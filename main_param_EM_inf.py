...
def main(dist, noise_dist, num_sim, num_samples, num_noise_samples, T, infinite):
    ...
    if dist=='normal':
        ...
        theta_w_list = [0.1, 0.2, 0.5, 1.0, 1.5] # radius of noise ambiguity set
    ...
    for i in range(T):
        W_hat[:,:,i] = Sigma_w_hat
        V_hat[:,:,i] = M_hat
    mu_w_hat_drlqc = np.tile(mu_w_hat, (T,1,1) )
    mu_v_hat_drlqc = np.tile(mu_v_hat, (T+1,1,1) )
    # ----------------------------
    if infinite==False:
    ...
    def perform_simulation(lambda_, noise_dist, dist_parameter, theta, idx_w, idx_v):
        for num_noise in num_noise_list:
            ...
            if use_lambda:
                path = "./results/{}_{}/finite/multiple/DRLQC/params_lambda/io_inf/".format(dist, noise_dist)
            else:
                path = "./results/{}_{}/finite/multiple/DRLQC/params_thetas/io_inf/".format(dist, noise_dist)
            ...
            drlqc = DRLQC(theta_w, theta, theta_x0, T, dist, noise_dist, system_data, mu_w_hat_drlqc, W_hat, x0_mean, x0_cov, x0_max, x0_min, mu_w, Sigma_w, w_max, w_min, v_max, v_min, mu_v, mu_v_hat_drlqc, V_hat, x0_mean_hat, x0_cov_hat, tol)
            
            if infinite:
                if use_optimal_lambda == True:
                    lambda_ = DRCE_lambda[idx_w][idx_v]
                drce = inf_DRCE(lambda_, theta_w, theta, theta_x0, T, dist, noise_dist, system_data, mu_w_hat, Sigma_w_hat, x0_mean, x0_cov, x0_max, x0_min, mu_w, Sigma_w, w_max, w_min, v_max, v_min, mu_v, mu_v_hat,  M_hat, x0_mean_hat, x0_cov_hat, use_lambda, use_optimal_lambda)
                if use_optimal_lambda == True:
                    lambda_ = WDRC_lambda[idx_w][idx_v]
                wdrc = inf_WDRC(lambda_, theta_w, T, dist, noise_dist, system_data, mu_w_hat, Sigma_w_hat, x0_mean, x0_cov, x0_max, x0_min, mu_w, Sigma_w, w_max, w_min, v_max, v_min, mu_v, mu_v_hat, M_hat, x0_mean_hat, x0_cov_hat, use_lambda, use_optimal_lambda)
                lqg = inf_LQG(T, dist, noise_dist, system_data, mu_w_hat, Sigma_w_hat, x0_mean, x0_cov, x0_max, x0_min, mu_w, Sigma_w, w_max, w_min, v_max, v_min, mu_v, mu_v_hat, M_hat , x0_mean_hat, x0_cov_hat)
            else:
                if use_optimal_lambda == True:
                    lambda_ = DRCE_lambda[idx_w][idx_v]
            ...
            #Save all raw data
            if use_lambda:
                rawpath = "./results/{}_{}/finite/multiple/DRLQC/params_lambda/io_inf/raw/".format(dist, noise_dist)
            else:
                rawpath = "./results/{}_{}/finite/multiple/DRLQC/params_thetas/io_inf/raw/".format(dist, noise_dist)
    ...
    if use_lambda:
        print("Now use : python plot_params4_EM_inf.py --use_lambda --dist "+ dist + " --noise_dist " + noise_dist)
    else:
        print("Now use : python plot_params4_EM_inf.py --dist "+ dist + " --noise_dist " + noise_dist)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--infinite', required=False, action="store_true") #infinite horizon, if flagged
