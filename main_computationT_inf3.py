...
controller instantiation and backward() method for the DRCE, inf_DRCE,
and DRLQC controllers as a function of the simulation horizon T.
We use lambda = 10.0 and use_lambda=True.
For each time horizon T (e.g., 5, 10, …, 60 for DRCE/inf_DRCE; for DRLQC, only T ≤ 40 are measured),
the code:
  1. Generates data and runs EM once to obtain nominal parameters.
  2. Prepares the nominal parameters.
  3. For a fixed number of repetitions (num_reps), re-generates the data and nominal parameters
     (using a different random seed) and then re-instantiates each controller
     and measures the time from before instantiation to after backward() returns.
     For DRLQC, the measured time includes solve_sdp() and backward().
...
from controllers.DRLQC import DRLQC   # <--- Added DRLQC import
...
# For each horizon T, we generate data and run EM once for each repetition.
# Then, for num_reps repetitions, we re-generate the data and nominal parameters (using a different random seed)
# and measure the time (instantiation + backward) for DRCE, inf_DRCE, and DRLQC 
# (for DRLQC, also include solve_sdp()).
def measure_backward_time_for_horizon(T, num_reps, dist, noise_dist, use_lambda=True):
    # System Initialization (fixed for all reps)
    nx = 10; nu = 10; ny = 10
    temp = np.ones((nx, nx))
    ...
    # Disturbance and noise distribution parameters (deterministic)
    if dist=="normal":
    ...
    # Lists to accumulate timing measurements over different random scenarios
    drce_times = []
    inf_drce_times = []
    drlqc_times = []
    
    for rep in range(num_reps):
        # Set a new random seed for each repetition to generate different nominal distributions.
        np.random.seed(2024 + T + rep)
        
        # --- Generate Data and run EM for this repetition ---
        N = 50
        ...
        # --- Measure time including instantiation and backward() for each controller ---
        # DRCE 
        ...
        # DRLQC (only measure if T <= 40)
        if T <= 40:
            start = time.perf_counter()
            # For DRLQC we need to construct the batch matrices:
            tol = 1e-2
            ...
            for i in range(T+1):
                V_hat[:,:,i] = M_hat
            drlqc_rep = DRLQC(fixed_theta_w, fixed_theta_v, theta_x0, T, dist, noise_dist,
    ...
    if T <= 40 and len(drlqc_times) > 0:
        avg_drlqc_time = np.mean(drlqc_times)
        std_drlqc_time = np.std(drlqc_times)
    else:
        avg_drlqc_time = np.nan
        std_drlqc_time = np.nan
    
    return avg_drce_time, std_drce_time, avg_inf_drce_time, std_inf_drce_time, avg_drlqc_time, std_drlqc_time

def main(dist, noise_dist, num_sim, num_reps):
    # For DRCE and inf_DRCE, we measure T = 5,10,...,60.
    horizon_list = list(range(5, 61, 5))
    summary = {"T": [],
               "DRCE": {"mean": [], "std": []},
               "inf_DRCE": {"mean": [], "std": []},
               "DRLQC": {"mean": [], "std": []}}
    for T in horizon_list:
        ...
        (avg_drce_time, std_drce_time, avg_inf_drce_time, std_inf_drce_time,
         avg_drlqc_time, std_drlqc_time) = measure_backward_time_for_horizon(T, num_reps, dist, noise_dist, use_lambda=True)
        summary["T"].append(T)
        ...
        summary["DRLQC"]["mean"].append(avg_drlqc_time)
        summary["DRLQC"]["std"].append(std_drlqc_time)
    # Save in appropriate folder and file name.
    save_folder = "./results/backward_time_experiment_lambda/"
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)
    save_data(os.path.join(save_folder, f"backward_time_costs_exp_time_5_{dist}_{noise_dist}.pkl"), summary)
