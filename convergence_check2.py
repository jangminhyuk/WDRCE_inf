...
def dr_kf_solve_measurement_update(Sigma_x_minus, C, Sigma_v_nom, theta, delta=1e-6):
    ...
    constraints = [
        ...
        Sigma_v_var >> delta * np.eye(m)  # This ensures positive definiteness
    ]
...
def run_dr_kf_once(n=10, m=10, steps=100, theta=0.5, N_samples=20, dist_type="normal"):
    # Generate a random system (for testing, detectability/stabilizability are not strictly enforced)
    A = np.random.randn(n, n)
    ...
    # Compute the difference between consecutive posteriors to check convergence
...
if __name__=="__main__":
    ...
    theta_values = [0.1, 1, 10, 100, 1000, 10000]
    ...
    for theta in theta_values:
        ...
        print(f"  Convergence rate: {convergence_rate:.1f}% of experiments converged")
    ...
    print("\n=== Final Summary Across All Theta Values ===")
    print("Theta\t\tAvg Final Norm\t\tAvg Posterior Trace\tConvergence Rate (%)")
    for summary in theta_summary:
        print(f"{summary['theta']:<8}\t{summary['avg_norm']:<18.4e}\t{summary['avg_trace']:<22.4e}\t{summary['convergence_rate']:<5.1f}")
