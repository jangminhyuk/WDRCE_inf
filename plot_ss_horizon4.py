...
# -*- coding: utf-8 -*-
"""
This file plots the results from the time-horizon experiment (experiment 4).
...
def main(dist, noise_dist, use_lambda_flag):
    if use_lambda_flag:
        base_path = "./results/time_horizon_experiment_4_lambda/"
    else:
        base_path = "./results/time_horizon_experiment_4/"
    # Note: the file name now includes "exp4" to reflect the experiment with four controllers.
    data_path = os.path.join(base_path, f"time_horizon_costs_exp4_{dist}_{noise_dist}.pkl")
    ...
    # plt.errorbar(T_values, LQG_infinite_mean, yerr=LQG_infinite_std, fmt='-s', label='LQG (infinite)')
    # plt.errorbar(T_values, DRCE_finite_mean,   yerr=DRCE_finite_std,   fmt='-^', label='DRCE (finite)')
    # plt.errorbar(T_values, DRCE_infinite_mean, yerr=DRCE_infinite_std, fmt='-d', label='DRCE (infinite)')
    
    plt.xlabel("Time Horizon (T)", fontsize=14)
    plt.ylabel("Time–Averaged Control Cost", fontsize=14)
    plt.title(f"Time–Averaged Cost vs Time Horizon", fontsize=16)
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    save_filename = os.path.join(base_path, f"time_horizon_plot_exp4_{dist}_{noise_dist}.pdf")
