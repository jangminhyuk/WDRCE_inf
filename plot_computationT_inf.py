...
It loads the summary data saved by generate_backward_time_experiment.py and
...
def main(dist, noise_dist):
    ...
    data_path = os.path.join(base_path, f"backward_time_costs_{dist}_{noise_dist}.pkl")
    ...
    plt.errorbar(T_values, DRCE_mean, yerr=DRCE_std, fmt='-o', label='DRCE backward time')
    plt.errorbar(T_values, inf_DRCE_mean, yerr=inf_DRCE_std, fmt='-s', label='inf_DRCE backward time')
    plt.xlabel("Time Horizon (T)", fontsize=14)
    plt.ylabel("Average Backward Computation Time (s)", fontsize=14)
    plt.title(f"Backward Computation Time vs Time Horizon\n(dist={dist}, noise_dist={noise_dist})", fontsize=16)
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(base_path, f"backward_time_plot_{dist}_{noise_dist}.pdf"), dpi=300)
    plt.show()
