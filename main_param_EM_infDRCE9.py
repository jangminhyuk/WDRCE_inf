...
For each parameter combination, the nominal distribution is generated
num_experiments=10 times (i.e. by re-running data generation and EM estimation)
and the forward simulation results are averaged. The saved file names and data format
remain unchanged so that your previous plotting code continues to work.
"""
...
def main(dist, noise_dist, num_sim, num_samples, num_noise_samples, T, infinite):
    ...
    output_J_LQG_mean = []       # Now inf_LQG cost.
    output_J_LQG_std = []
    ...
    N = 50
    # (Data generation here is only used to check dimensions.)
    ...
    # Create a results directory (same for all simulations).
    if use_lambda:
    ...
    # Modified simulation function: It now repeats the entire nominal distribution
    # generation (data generation and EM estimation) num_experiments times.
    def perform_simulation(lambda_, noise_dist, dist_parameter, theta, idx_w, idx_v):
        rep_costs_lqg = []
        rep_costs_drce = []
        rep_costs_inf_drce = []
        for rep in range(num_experiments):
            # Set new random seed for each repetition.
            ...
            # Generate new data and estimate nominal parameters via EM.
            ...
            # For finite-horizon controllers, tile the nominal parameters;
            # for infinite controllers we use the steady-state estimates.
            ...
            # Use inf_LQG (infinite-horizon LQG) with the steady-state nominal parameters.
            ...
            # Finite-horizon DRCE uses tiled parameters.
            drce = DRCE(lambda_local, theta_w_local, theta, theta_x0, T, dist, noise_dist,
            ...
            # Infinite-horizon DRCE uses steady-state nominal parameters.
            ...
            for i in range(num_sim):
                ...
                out_lqg_list.append(out_inf_lqg)
                if i % 50 == 0:
                    print("Simulation #", i, '| cost (inf_LQG):', out_inf_lqg['cost'][0])
            avg_lqg = np.mean([out['cost'][0] for out in out_lqg_list])
            
            print("Running finite-horizon DRCE Forward step ... (rep {})".format(rep))
            for i in range(num_sim):
                out_drce = drce.forward()
                out_drce_list.append(out_drce)
                if i % 50 == 0:
                    print("Simulation #", i, '| cost (finite DRCE):', out_drce['cost'][0])
            avg_drce = np.mean([out['cost'][0] for out in out_drce_list])
            
            print("Running infinite-horizon DRCE Forward step ... (rep {})".format(rep))
            for i in range(num_sim):
                out_inf_drce = inf_drce.forward()
                out_inf_drce_list.append(out_inf_drce)
                if i % 50 == 0:
                    print("Simulation #", i, '| cost (infinite DRCE):', out_inf_drce['cost'][0])
            avg_inf_drce = np.mean([out['cost'][0] for out in out_inf_drce_list])
            
            rep_costs_lqg.append(avg_lqg)
            rep_costs_drce.append(avg_drce)
            rep_costs_inf_drce.append(avg_inf_drce)
        
        # Average over the num_experiments repetitions.
        mean_lqg = np.mean(rep_costs_lqg)
        std_lqg = np.std(rep_costs_lqg)
        mean_drce = np.mean(rep_costs_drce)
        std_drce = np.std(rep_costs_drce)
        ...
        output_J_LQG_mean.append(mean_lqg)
        output_J_LQG_std.append(std_lqg)
        output_J_DRCE_mean.append(mean_drce)
        output_J_DRCE_std.append(std_drce)
        output_J_inf_DRCE_mean.append(mean_inf_drce)
        output_J_inf_DRCE_std.append(std_inf_drce)
        
        # Save the averaged cost for this parameter combination.
        theta_v_str = f"_{str(theta).replace('.', '_')}"
        theta_w_str = f"_{str(theta_w_local).replace('.', '_')}"
        if use_lambda:
            save_data(path + 'drce_finite_' + str(lambda_local) + 'and' + theta_v_str + '.pkl', mean_drce)
            save_data(path + 'drce_infinite_' + str(lambda_local) + 'and' + theta_v_str + '.pkl', mean_inf_drce)
        else:
            save_data(path + 'drce_finite' + theta_w_str + 'and' + theta_v_str + '.pkl', mean_drce)
            save_data(path + 'drce_infinite' + theta_w_str + 'and' + theta_v_str + '.pkl', mean_inf_drce)
        save_data(path + 'lqg.pkl', mean_lqg)
        
        # (Optional) Save raw data for this parameter combination if desired.
        rawpath = os.path.join(path, "raw")
        if not os.path.exists(rawpath):
            os.makedirs(rawpath)
        # (Raw data saving code can be added here if needed.)
    ...
    # Run the simulation in parallel over parameter combinations.
